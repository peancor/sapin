<script lang="ts">
	...
	let hasInitializedOpenGroups = $state(false);

	$effect(() => {
		if (hasInitializedOpenGroups) return;
		openGroupIds = [...initiallyOpenGroupIds];
		hasInitializedOpenGroups = true;
	});

	function selectedCountForGroup(group: { tools: LessonAgentToolPresentationItem[] }) {
		return group.tools.filter((tool) => selectedIds.has(tool.id)).length;
	}

	function handleGroupToggle(groupId: string, event: Event) {
		const details = event.currentTarget as HTMLDetailsElement;
		openGroupIds = details.open
			? [...new Set([...openGroupIds, groupId])]
			: openGroupIds.filter((candidate) => candidate !== groupId);
	}
</script>
...
{:else if compact}
	<div class="space-y-3">
		{#each groupedTools as group (group.id)}
			...
			<details
				class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950/20"
				open={openGroupIds.includes(group.id)}
				ontoggle={(event) => handleGroupToggle(group.id, event)}
			>
				<summary
					class="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 px-4 py-3 marker:hidden"
				>
					<div class="min-w-0">
					...
					<span class="text-primary-700 dark:text-primary-300 text-xs font-medium">
						{openGroupIds.includes(group.id) ? 'Ocultar' : 'Ver tools'}
					</span>
				</summary>
				...
				<div class="border-t border-gray-200 dark:border-gray-800">
					{#each group.tools as tool (tool.id)}
						<label
							class="flex gap-3 border-b border-gray-100 px-4 py-3 last:border-b-0 hover:bg-gray-50 dark:border-gray-900 dark:hover:bg-gray-900/40"
						>
							<input
								type="checkbox"
								class="text-primary-600 mt-0.5 h-4 w-4 rounded border-gray-300"
