...
export default class DBAgentMessageUtils {
	...
	static async updateAgentMessage(
		...
		await db.update(schema.agentMessage).set(data).where(eq(schema.agentMessage.id, messageId));
	}
	...
	static async getAgentMessagesAsModelMessages(chatId: string): Promise<ModelMessage[]> {
		...
		for (const msg of rawMessages) {
			if (msg.role === 'user') {
				const attachments = await AgentMessageAttachmentService.getModelImageAttachments(msg.id);
				if (attachments.length > 0) {
					result.push({
						role: 'user',
						content: [
							{ type: 'text', text: msg.textContent ?? '' },
	...
	static async updateToolCall(
		...
		await db.update(schema.agentToolCall).set(data).where(eq(schema.agentToolCall.id, toolCallId));
	}
