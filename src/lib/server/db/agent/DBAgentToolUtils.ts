	...
	static async seedBuiltinTools() {
		const builtinTools = [
			...
			},
			{
				...
				displayName: 'Mostrar Grafico Educativo',
				description:
					'Genera y muestra un grafico educativo en el chat. Usa mode=\"formula\" para y=f(x) o mode=\"data\" para series de puntos [x,y].',
				category: 'ui',
				parametersSchema: JSON.stringify({
					type: 'object',
					properties: {
						mode: { type: 'string', enum: ['formula', 'data'] },
						title: { type: 'string', description: 'Titulo opcional del grafico' },
						formula: {
							type: 'object',
							properties: {
								expression: { type: 'string', description: 'Formula de la forma y=f(x)' },
			...
			},
			{
				...
				description:
					'Muestra en el chat una imagen subida como recurso compartido de la actividad actual. Usa resourceName para buscar la imagen por nombre dentro de la actividad.',
				category: 'ui',
				parametersSchema: JSON.stringify({
					type: 'object',
					properties: {
