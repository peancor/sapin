...
import { syncForProgress as syncLtiGradeForProgress } from '$lib/server/lti/gradeSync';
...
async function syncCourseProgressSummary(
	...
	const [activityRecords, courseActivities, existingSummary] = await Promise.all([
		...
		db
			...
			.where(
				and(eq(courseProgressSummary.userId, userId), eq(courseProgressSummary.courseId, courseId))
			)
			.get()
	]);
...
export async function markActivityCompleted(input: ProgressWriteInput): Promise<void> {
	...
	await syncLtiGradeForProgress({
		...
		scoreNormalized: existing.scoreNormalized,
		completedAt: completionTimestamp
	});
}
