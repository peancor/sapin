...
export const agentMessageAttachmentKind = {
	IMAGE: 'image'
} as const;

export const agentMessageAttachmentStatus = {
	PENDING: 'pending',
	ATTACHED: 'attached',
	REJECTED: 'rejected'
} as const;

export const agentMessageAttachmentSafetyStatus = {
...
// ============================================
// ADJUNTOS DE MENSAJES AGÉNTICOS
// ============================================

export const agentMessageAttachment = sqliteTable(
	'agent_message_attachment',
	{
		...
		messageId: text('message_id').references(() => agentMessage.id, { onDelete: 'cascade' }),
		fileStorageId: text('file_storage_id')
			.notNull()
			.references(() => fileStorage.id, { onDelete: 'cascade' }),
		kind: text('kind').notNull().default('image'),
		status: text('status').notNull().default('pending'),
		safetyStatus: text('safety_status').notNull().default('not_checked'),
		sequenceOrder: integer('sequence_order').notNull().default(0),
		width: integer('width'),
	...
	(table) => [
		index('agent_message_attachment_chat_idx').on(table.chatId),
		index('agent_message_attachment_message_idx').on(table.messageId),
		index('agent_message_attachment_file_idx').on(table.fileStorageId),
		index('agent_message_attachment_uploaded_by_idx').on(table.uploadedBy),
		index('agent_message_attachment_status_idx').on(table.status)
	]
);
...
export const agentToolCall = sqliteTable(
	'agent_tool_call',
	{
		...
		toolDefinitionId: text('tool_definition_id').references(() => agentToolDefinition.id),
...
export const agentMessageRelations = relations(agentMessage, ({ one, many }) => ({
	...
	attachments: many(agentMessageAttachment)
}));

export const agentMessageAttachmentRelations = relations(agentMessageAttachment, ({ one }) => ({
	chat: one(chat, {
		fields: [agentMessageAttachment.chatId],
		references: [chat.id]
	}),
	message: one(agentMessage, {
		fields: [agentMessageAttachment.messageId],
		references: [agentMessage.id]
	}),
	file: one(fileStorage, {
		fields: [agentMessageAttachment.fileStorageId],
		references: [fileStorage.id]
	}),
	uploadedByUser: one(user, {
		fields: [agentMessageAttachment.uploadedBy],
		references: [user.id]
	})
}));
...
export type AgentMessageAttachment = typeof agentMessageAttachment.$inferSelect;
export type NewAgentMessageAttachment = typeof agentMessageAttachment.$inferInsert;
