import { db } from '.';
import { eq, and, asc, desc, count, avg, sql, inArray } from 'drizzle-orm';
...
export default class DBAgentUtils {
    ...
    static async seedGlobalTutor() {
        ...
        // Asignar todas las herramientas builtin al tutor global
        await this.seedBuiltinTools();
        await this.seedBuiltinUIComponents();
        const tools = await this.getActiveToolDefinitions();
        ...
        const existing = await db
            ...
            .where(
                and(
                    ...
                    eq(
                        schema.userInteractiveLearningChat.interactiveLearningChatId,
                        this.GLOBAL_TUTOR_ID
                    )
                )
            )
