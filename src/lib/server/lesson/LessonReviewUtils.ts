...
export function buildLessonReviewAttemptSummary(input: {
	...
	return {
		...
		definitionRevisionId: session.definitionRevisionId ?? null,
		definitionRevisionNumber: session.definitionRevisionNumber ?? null,
		bindingStatus: session.bindingStatus,
		isHistoricalApproximation: session.bindingStatus === 'backfilled_current',
		visitedBlocksCount,
