...
export async function validateSessionToken(token: string) {
	...
	// Cargar roles del usuario desde el nuevo sistema
	const now = new Date();
	const userRoles = await db
		.select({ role: table.role })
		.from(table.userRoleAssignment)
		.innerJoin(table.role, eq(table.userRoleAssignment.roleId, table.role.id))
		.where(
			and(
	...
	return { 
		...
		user: {
			...
			roles,
			highestRole,
			highestRoleLevel: highestRole?.level ?? 0
		}
	};
}
