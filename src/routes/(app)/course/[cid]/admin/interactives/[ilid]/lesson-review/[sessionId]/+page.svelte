			...
			>
				<div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
					<div class="min-w-0 flex-1">
						<div class="flex flex-wrap items-center gap-2">
							...
							<span
								class={`rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] uppercase ${statusClasses(data.detail.attempt)}`}
							>
								{statusLabel(data.detail.attempt)}
							</span>
							<span
								class={`rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] uppercase ${audienceBadgeClasses()}`}
							>
								{audienceLabel()}
							</span>
						</div>
						...
						{#if data.detail.student.audience === 'staff'}
							<p
								class="mt-3 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800 dark:border-sky-900/60 dark:bg-sky-950/25 dark:text-sky-200"
							>
								Intento de depuración del staff. Se muestra para validar la lectura de revisión,
								pero no cuenta dentro de la interpretación pedagógica del alumnado.
							</p>
						{/if}
						<div class="mt-4 flex flex-wrap gap-2">
							{#if data.detail.attempt.definitionRevisionNumber !== null}
								<span
									class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200"
								>
							...
							{#if data.detail.attempt.isHistoricalApproximation}
								<span
									class="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-200"
								>
						...
						{#if data.detail.attempt.isHistoricalApproximation}
							<p
								class="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-200"
							>
								Este intento fue ligado retroactivamente a la revisión publicada actual cuando se
								activó el versionado. La versión histórica exacta previa no estaba almacenada.
							</p>
						{/if}
					</div>
				</div>
				...
				<div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
					...
					<div class="rounded-2xl bg-slate-100/90 px-4 py-3 dark:bg-slate-800/80">
						...
						<p class="mt-1 text-base font-semibold text-slate-900 dark:text-white">
							{data.detail.attempt.checkRetryBlocks} reintentos · {data.detail.attempt
				...
				<div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
					...
					>
						...
						<p class="mt-1 text-sm text-slate-700 dark:text-slate-200">
							{formatDate(data.detail.attempt.startedAt)}
						</p>
					</div>
					<div
						class="rounded-2xl border border-slate-200/80 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50"
					>
						...
						<p class="mt-1 text-sm text-slate-700 dark:text-slate-200">
							{formatDate(data.detail.attempt.lastActiveAt)}
						</p>
					</div>
					<div
						class="rounded-2xl border border-slate-200/80 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50"
					>
						...
						<p class="mt-1 text-sm text-slate-700 dark:text-slate-200">
							{data.detail.attempt.currentBlockTitle}
						</p>
					</div>
					<div
						class="rounded-2xl border border-slate-200/80 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50"
					>
						...
						<p class="mt-1 text-sm text-slate-700 dark:text-slate-200">
							{data.detail.attempt.completedAt
								? formatDate(data.detail.attempt.completedAt)
								: 'Todavía abierto'}
						</p>
					</div>
				</div>
			...
			>
				...
				<div class="mt-5 space-y-3">
					{#each data.detail.history as historyAttempt (historyAttempt.sessionId)}
						<a
							href={resolve(
								`/course/${page.params.cid}/admin/interactives/${page.params.ilid}/lesson-review/${historyAttempt.sessionId}`
							)}
						...
						>
							<div class="flex flex-wrap items-center gap-2">
								<span
									class={`rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] uppercase ${statusClasses(historyAttempt)}`}
								>
				...
				>
					<div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
						<div class="min-w-0 flex-1">
							<div class="flex flex-wrap items-center gap-2">
								<span
									class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] text-slate-600 uppercase dark:bg-slate-800 dark:text-slate-300"
								>
									Visita {visit.visitNumber}
								</span>
								<span
									class={`rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] uppercase ${visitStatusClasses(visit)}`}
								>
									{visitStatusLabel(visit)}
								</span>
								<span
									class="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] text-slate-700 uppercase dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200"
								>
									{visit.blockKind}
								</span>
							</div>
						...
						{#if visit.branchTargetBlockId}
							<div
								class="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-700 dark:border-sky-900/60 dark:bg-sky-950/25 dark:text-sky-300"
							>
								<p class="font-medium">Transición</p>
								<p class="mt-1">
									{visit.branchLabel || 'Salida registrada'} → {visit.branchTargetBlockId}
								</p>
							</div>
						{/if}
					</div>
					...
					<div class="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(280px,0.9fr)]">
						<div
							class="rounded-[26px] border border-slate-200/80 bg-slate-50/80 px-4 py-4 dark:border-slate-800 dark:bg-slate-950/50"
						>
							{#if visit.contentSummary}
								...
								<div
									class="prose prose-sm dark:prose-invert mt-2 max-w-none text-slate-700 dark:text-slate-200"
								>
							...
							{#if visit.choice}
								...
								>
									...
									<p class="mt-2 text-sm text-slate-700 dark:text-slate-200">
										...
										{visit.choice.selectedLabel || 'Sin etiqueta'}
									</p>
									{#if visit.choice.selectedValue}
										<p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
											<strong>Valor:</strong>
											{visit.choice.selectedValue}
										</p>
									{/if}
								</div>
							{/if}
							...
							{#if visit.check}
								<div
									class={visit.contentSummary || visit.choice
										? 'mt-4 border-t border-slate-200/80 pt-4 dark:border-slate-800'
										: ''}
								>
									<p
									...
									{#if visit.check.feedback}
										<div
											class="prose prose-sm dark:prose-invert mt-3 max-w-none text-slate-700 dark:text-slate-200"
										>
											<!-- eslint-disable-next-line svelte/no-at-html-tags -->
							...
							{#if visit.agent}
								<div
									class={visit.contentSummary || visit.choice || visit.check
										? 'mt-4 border-t border-slate-200/80 pt-4 dark:border-slate-800'
										: ''}
								>
									<p
									...
									{#if visit.agent.extractionStatus === 'failed' || visit.agent.extractionStatus === 'missing_field' || visit.agent.extractionStatus === 'coerced'}
										<div
											class="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-200"
										>
											{visit.agent.extractionMessage ||
												'La salida estructurada de este bloque necesita revisión.'}
										</div>
									{/if}
											...
											{#each visit.agent.transcript as transcriptMessage (transcriptMessage.id)}
												...
												>
													<p
														class="text-[11px] font-semibold tracking-[0.18em] text-slate-500 uppercase dark:text-slate-400"
													>
														{messageRoleLabel(transcriptMessage)} · {formatDate(
															transcriptMessage.createdAt
														)}
													</p>
													<div
														class="prose prose-sm dark:prose-invert mt-2 max-w-none text-slate-700 dark:text-slate-200"
						...
						<div class="space-y-3">
							<div class="rounded-[26px] bg-slate-100/90 px-4 py-4 dark:bg-slate-800/80">
								<p
									class="flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] text-slate-500 uppercase dark:text-slate-400"
								>
									<Route class="h-4 w-4" />
									Recorrido
								</p>
								<p class="mt-2 text-sm text-slate-700 dark:text-slate-200">
									Bloque <strong>{visit.blockId}</strong> dentro del intento #{data.detail.attempt
										.attemptNumber}.
								</p>
							</div>
							...
							{#if visit.branchTargetBlockId}
								<div class="rounded-[26px] bg-sky-50 px-4 py-4 dark:bg-sky-950/20">
									<p
										class="flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] text-sky-700 uppercase dark:text-sky-300"
									>
										<GitBranch class="h-4 w-4" />
										Rama tomada
									</p>
									<p class="mt-2 text-sm text-sky-800 dark:text-sky-200">
										{visit.branchLabel || 'Transición registrada'} hacia
										<strong>{visit.branchTargetBlockId}</strong>.
									</p>
								</div>
							{/if}
							...
							{#if visit.blockKind === 'check'}
								<div class="rounded-[26px] bg-amber-50 px-4 py-4 dark:bg-amber-950/20">
									<p
										class="flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] text-amber-700 uppercase dark:text-amber-300"
									>
							...
							{#if visit.blockKind === 'end'}
								<div class="rounded-[26px] bg-emerald-50 px-4 py-4 dark:bg-emerald-950/20">
									<p
										class="flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] text-emerald-700 uppercase dark:text-emerald-300"
									>
