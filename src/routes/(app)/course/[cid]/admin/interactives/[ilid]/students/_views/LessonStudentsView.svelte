<script lang="ts">
	...
	function exportToCSV(): void {
		...
		data.lessonStudents.forEach((row) => {
			rows.push(
				[
					...
					<TableBodyRow>
						...
						<TableBodyCell>
							...
							{#if row.student.alias}
								<p class="text-xs italic text-gray-500 dark:text-gray-400">{row.student.alias}</p>
							{/if}
						...
						<TableBodyCell>
							<a
								href={resolve(`/course/${courseId}/admin/interactives/${interactiveId}/students/${row.student.id}`)}
