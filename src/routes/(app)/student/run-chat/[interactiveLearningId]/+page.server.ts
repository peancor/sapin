...
export const load = (async (event) => {
    ...
    if (interactiveChat.interactive_learning.status === 'closed') {
        error(403, 'Esta actividad está cerrada y no admite nuevas interacciones. Puedes consultar tu historial si ya participaste anteriormente.');
    }
    ...
    const response = await event.fetch(`/api/interactive-chat/${interactiveChat.interactive_learning.id}/chat`, {
        method: 'POST',
        headers: {
