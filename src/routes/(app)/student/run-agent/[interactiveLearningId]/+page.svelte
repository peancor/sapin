<script lang="ts">
	...
	onMount(() => {
		...
		const redirectTimer = window.setTimeout(() => {
			goto(resolve(`/agent-chat/${data.activityId}/c/${data.chatId}`), { invalidateAll: true });
		}, 2000);
...
{:else}
	<div
		class="flex min-h-screen items-center justify-center bg-linear-to-br from-emerald-50 via-cyan-50 to-sky-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900"
	>
		<div class="pointer-events-none absolute inset-0 overflow-hidden">
			<div
				class="absolute top-1/4 left-1/4 h-64 w-64 animate-pulse rounded-full bg-emerald-400/10 blur-3xl dark:bg-emerald-500/5"
			></div>
			<div
				class="absolute right-1/4 bottom-1/4 h-80 w-80 animate-pulse rounded-full bg-cyan-400/10 blur-3xl dark:bg-cyan-500/5"
				style="animation-delay: 1s;"
			></div>
			<div
				class="absolute top-1/2 right-1/3 h-48 w-48 animate-pulse rounded-full bg-sky-400/10 blur-3xl dark:bg-sky-500/5"
		...
		<div class="relative z-10 mx-4 w-full max-w-md">
			...
			>
				<div
					class="relative flex h-32 items-center justify-center overflow-hidden bg-linear-to-r from-emerald-500 via-cyan-500 to-sky-500"
				>
				...
				<div class="space-y-6 p-8 text-center">
					...
					<div class="relative">
						<div class="h-2 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
							<div
								class="animate-progress h-full rounded-full bg-linear-to-r from-emerald-500 via-cyan-500 to-sky-500"
							></div>
						</div>
					</div>
