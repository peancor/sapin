...
export const PUT: RequestHandler = async ({ locals, params, request }) => {
    ...
    try {
        ...
        if (body.parametersSchema !== undefined) {
            try { JSON.parse(body.parametersSchema); updates.parametersSchema = body.parametersSchema; }
        ...
        if (body.responseSchema !== undefined) {
            if (body.responseSchema === null || body.responseSchema === '') {
                ...
                try { JSON.parse(body.responseSchema); updates.responseSchema = body.responseSchema; }
                catch { return json({ error: 'responseSchema no es JSON válido' }, { status: 400 }); }
            }
        }
        if (body.executorConfig !== undefined) {
            try { JSON.parse(body.executorConfig); updates.executorConfig = body.executorConfig; }
