...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		const session = await LessonService.startPreviewSession({
			interactiveLearningId: params.ilid,
			userId: user.id,
			userRoleLevel: user.highestRoleLevel,
			courseId: payload.courseId,
			previewMode: payload.previewMode === 'draft' ? 'draft' : 'published'
		});
		...
		console.error('[lesson] preview session create error', error);
