...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		if (!payload.courseId || !payload.blockId) {
			return json({ error: 'Missing courseId or blockId' }, { status: 400 });
		}
		...
		const session = await LessonService.jumpPreviewSessionToBlock({
		...
		console.error('[lesson-studio-debug] jump preview session error', error);
